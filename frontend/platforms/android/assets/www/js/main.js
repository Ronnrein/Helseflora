$(document).ready(function(){
    ...
    function fillUserOrders(){
        $("#userOrderList").html("");
        var user = User.get();
        $.getJSON(app.serverUrl, {a: "getAllOrders", userid: user.id, token: user.token})
            .done(function(data){
                console.log(data);
                if(data !== 0 && data !== 2){
                    $.each(data, function(i, order){
                        ...
                        $.each(order.items, function(j, item){
                            str += item.amount+" "+item.name+", ";
                        });
                        str = str.substring(0, str.length - 2);
                        $("#userOrderList").append("<li><h3>Ordre fra: "+order.placed+"</h3><p>"+str+"</p></li>");
                    });
                    $("#userOrderList").listview("refresh");
                }
            });
    }
    ...
    function setUserStatus(loggedIn){
        if(loggedIn){
            ...
            if(user.access == 1){
                $("#adminNavButton").parent().show();
            } else{
                $("#adminNavButton").parent().hide();
            }
            $("#userPage").page();
            setTimeout(function(){
                fillUserOrders();
    ...
    // Show new page with html content (Used for php errors)
    ...
    $("#loginForm").submit(function(e){
        ...
        $.post(app.serverUrl, $(this).serialize())
            .done(function(data){
                console.log(data);
                if(data !== 0){
                    User.set(data.id, data.username, data.name, data.email, data.token, data.access);
                    setUserStatus(true);
                    hideSpinner();
                    $.mobile.changePage($("#mainPage"), "slide");
                } else{
    ...
    $("#editUserForm").submit(function(e){
        ...
        $.post(app.serverUrl, $(this).serialize())
            .done(function(data){
                $("#editPassword").val("");
                hideSpinner();
                app.showAlert("Informasjon oppdatert", "Suksess");
            });
        return false;
    });
        ...
        $.post(app.serverUrl, $(this).serialize())
            .done(function(data){
                ...
                if(data !== 0){
                    ...
                    $.mobile.changePage($("#mainPage"), "slide");
                    app.showAlert("Bruker laget!", "Suksess");
                }
                else{
                    app.showAlert("Noe gikk galt", "Feil");
                }
            });
        return false;
    });
